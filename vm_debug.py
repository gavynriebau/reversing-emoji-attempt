...
# This is a slightly modified version of vm.py that writes some extra debugging information to a file.
...
class VM:
  ...
  def step(self):
    ...
    self.debug_file.write("(Py) Processing instruction '{}'\n".format(cur_ins[0]))
  ...
  def if_zero(self):
    last = self.stack[-1]
    if last == 0:
      self.debug_file.write("if_zero was zero\n")
      while self.rom[self.instruction_pointer] != '😐':
        if self.rom[self.instruction_pointer] in ['🏀', '⛰']:
          break
        self.step()
    else:
      self.debug_file.write("if_zero was not zero '{}'\n".format(last))
  ...
  def jump_top(self):
    next = self.stack.pop()
    self.debug_file.write("Jump top to '{}'\n".format(next))
  ...
  def xor(self):
    a = self.stack.pop()
    b = self.stack.pop()
    self.debug_file.write("xor {} with prime {}\n".format(b, a))
